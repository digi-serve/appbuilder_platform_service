module.exports = function FNModelNetsuite({ ABModelPlugin }) {
   ...
   const CONCURRENCY_LIMIT_MAX =
      process.env["NETSUITE_CONCURRENCY_LIMIT"] || 35;
   // {int}
   // This is the number of parallel operations we want to limit ourselves to
   // so we avoid trippig NetSuit's CONCURRENCY_LIMIT_EXCEEDED.
   ...
   setInterval(() => {
      ...
      if (displayLog.length > 0) {
         console.log("=== Plugin: Netsuite Concurrency ====");
   ...
   return class ABModelAPINetsuite extends ABModelPlugin {
      constructor(object) {
         super(object);
      ...
      parseCondition(condition, userData, req) {
         ...
         switch (condition.rule) {
            ...
            case "is_current_date":
               operator = "BETWEEN";
               var datetimerange = this.AB.rules
                  .getUTCDayTimeRange()
                  .split("|");
